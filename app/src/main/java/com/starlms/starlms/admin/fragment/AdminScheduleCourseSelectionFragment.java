    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        adapter.setOnItemInteractionListener(new AdminCourseAdapter.OnItemInteractionListener() {
            ...
            public void onItemClick(CourseWithTeacher courseWithTeacher) {
                // Navigate to the Session Management screen for this course
                FragmentTransaction transaction = getParentFragmentManager().beginTransaction();
                transaction.replace(R.id.fragment_container, AdminSessionManagementFragment.newInstance(courseWithTeacher.getCourse().getCourseId(), courseWithTeacher.getCourse().getName()));
                transaction.addToBackStack(null); // Allows user to press back to return to course list
                transaction.commit();
            }
